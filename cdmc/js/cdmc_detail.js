...
function dataUpdate() {
    ...
    document.getElementById('index').textContent = "选手ID：";

    //积分排名相关
    document.getElementById('rank').textContent = '#' + data.members[rank].rank;
    document.getElementById('mmr').textContent = data.members[rank].displayPoints;
    //赛事数据相关
    ...
    fetch('/data_points/data.json')
        ...
        .then(jsonData => {
            ...
            function drawChart() {
                ...
                chartData.addColumn('number', '积分');
                chartData.addColumn({ type: 'string', role: 'annotation' });
                chartData.addColumn({ type: 'string', role: 'annotation' });
                chartData.addRow(["初始", 0, '0', '初始']);
                for (var i = player.historyPoints.length; i > 0; i--) {
                    var join = '';
                    ...
                    var point = player.historyPoints[i - 1] + player.extraPoints;
                    chartData.addRow([data.tournaments[i - 1].desc, point, String(point), join]);
                }
