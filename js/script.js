document.addEventListener('DOMContentLoaded', async () => {
    if (!DataService.isReady()) await DataService.load();

    const params = new URLSearchParams(window.location.search);
    const season = params.get('season') || 'latest';
    const seasonData = DataService.getSeasonData(season);
    const rank = Math.min(Math.max(parseInt(params.get('rank'), 10) || 1, 1), seasonData.members.length) - 1;
    const player = seasonData.members[rank];
    ...
    player.showInfo.historyResult.forEach(item => {
        ...
        row.insertCell(0).textContent = tour?.date || '';
